...
function countVowels(str) {
  const numberOfVowels = str.match(/[aeiouAEIOUY]/gi);
  return numberOfVowels ? numberOfVowels.length : 0;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < splitSentence.length; i += 1) {
    if (splitSentence[i].length > wordResult.length) {
      wordResult = splitSentence[i];
    }
  }
  return wordResult;
}
...
function reverseWords(str) {
  ...
  const wordsRevers = words.map((word) => word.split('').reverse().join(''));
  const strRevers = wordsRevers.join(' ');
...
function extractNameFromTemplate(value) {
  const extract = value.match(/Hello, (.+?)!/);
...
function unbracketTag(str) {
  const unbrackets = str.substring(1, str.length - 1);
  return unbrackets;
}
...
function encodeToRot13(str) {
  const symbolsNormal = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const symbolsRot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  return str.replace(/[a-z]/gi, (i) => symbolsRot13[symbolsNormal.indexOf(i)]);
}
